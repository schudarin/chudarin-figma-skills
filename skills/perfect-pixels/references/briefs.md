...
2. **Skills to load, minimal stack, in order.** `figma-use`, `figma-house-rules`, and exactly one
   visual skill for the task type (`SKILL.md`, "Minimum skill stack") — never the whole shelf.
   *Example: "Load, in order: figma-use, figma-house-rules, mobile-app-ui-design. Do not load
...
Load, in order: figma-use, figma-house-rules, [one visual skill for this task type — see
...
Load: figma-use, figma-house-rules. No visual skill — there is no design judgment in this task.
