...
- **Claude Code:** put the folder in `~/.claude/skills/figma-house-rules/` (available in
  every project) or in `<project>/.claude/skills/figma-house-rules/` (that project only).
- **Codex:** put it in `~/.agents/skills/figma-house-rules/` or `<repo>/.agents/skills/`.
  Invoke explicitly with `$figma-house-rules`; implicit invocation is on.
