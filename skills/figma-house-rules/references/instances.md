---
name: figma-house-rules/instances
...
**Principle:** Composite instance-override IDs (`I<id>;<suffix>;...`) obtained by a read-only call (`query('TEXT')` etc.) resolve reliably via `getNodeByIdAsync` within THE SAME call, but systematically return `null` / "not found" in the NEXT separate `use_figma` call — not occasionally (see the neighbouring gotcha `nested-instance-id-flaky-right-after-page-switch` about random flakiness) but **always**, if the node lies inside a remote/shared-library instance (a shared Sidebar, Navigation bar, language selector, pagination and similar components reused across the whole file). Local (non-remote) components aren't affected — in the same session neighbouring IDs from NON-remote instances resolved normally between calls.
...
**Principle:** If a single-locale assembly is needed but the only instance with the right STRUCTURE of nested sub-components (e.g. real `BadgeFilter` / `Text button` instances, not simplified frame clones without TEXT children) exists only in ANOTHER locale — don't clone that instance whole counting on patching every text node inside pointwise: it's easy to miss a node outside the focus area (e.g. the labels of the toolbar row itself, not part of the target sub-block with chips).
...
**Principle:** `rootInstance.detachInstance()` detaches only the root node itself (INSTANCE → FRAME) — nested sub-components (e.g. reusable `Breadcrumbs`/`Tabs`/`PageHeader`-like blocks inside a larger composite component) remain full INSTANCE nodes. `appendChild`/`remove` on descendants of such a nested instance still fail with `"Cannot move node. New parent is an instance or is inside of an instance"` / `"Removing this node is not allowed"`, even if the root parent is already detached.
...
```
Verified on a production admin dashboard, one of the list sections — a `Navigation bar/Admin` component (breadcrumb + title + tabs): both the nested `text header` (a `PageHeader` analogue) and the nested `tabs` row (a `Tabs` analogue) needed a separate detach each — detaching only the root would have sufficed had the component's composition not reused nested shared DS components inside itself.

### instance-subtree-remove-blocked-toggle-visible-instead
**Principle:** `.remove()` on a node lying inside someone else's INSTANCE subtree (an override child inside e.g. a `modal header` INSTANCE — not the root of a cloned composition but a nested widget) throws `Error: Removing this node is not allowed`, even if the top-level clone itself (`Modal-standart` etc.) is an ordinary FRAME, not an INSTANCE. The same class of restriction as in `nested-instance-boundaries-need-individual-detach` (append/remove across nested instance boundaries), but here solved MORE SIMPLY — without a detach: if the goal is just to hide an irrelevant/unused sub-element of a composite DS component (not to embed new content in its place), `node.visible = false` is a normal, supported override that breaks nothing.
...
```
Verified on a production admin dashboard, one of the sections — an attempt to add a 48 px reserve (`copy-icon-reserve`) inside a live `Content row-text` instance failed on `appendChild`; after `detachInstance()` the same structural edit passed normally.

### resize-silently-ignored-on-fill-sized-instance-nested-text
**Principle:** On a TEXT node lying INSIDE a live (not detached) instance and taking part in auto-layout as `layoutSizingHorizontal='FILL'`, calling `node.layoutSizingHorizontal='FIXED'; node.resize(W, H)` throws no error but also doesn't change the actually rendered width — a repeat `.width` read (including in a SEPARATE, definitely non-stale next `use_figma` call) shows the previous FILL-computed value, not the requested `W`. Unlike the ordinary auto-layout gotcha (`layoutsizing-fill-silent-noop-primary-axis`), the simple `constraints` workaround doesn't help here — the node lies inside an instance subtree where a structural override of text sizing properties apparently isn't applied by this API path at all.
...
Verified on a production admin dashboard, one of the sections — the `value` text (`#2`) inside a `Content row-text` instance consistently returned a 406 px width on attempts to set 47/92/145/226 px in five different calls; after `detachInstance()` the same `resize()` worked first time.
...
**Principle:** If the work plan on one header/shell instance includes BOTH actions — (a) toggle a boolean component property (e.g. `Show tabs`) and (b) detach a nested child instance (e.g. for reordering children) — the order is strictly important: toggle the boolean prop FIRST, before any detach. Detaching a nested instance cascades the detach to the instance ancestors (see the gotcha above) — the parent header turns from an INSTANCE into a FRAME, and `setProperties()` on it for boolean/variant props becomes entirely unavailable after that (an ordinary FRAME has no `componentProperties`).
...
```
Verified on a production admin dashboard, one of the sections (`actions group`, `COMPONENT_SET 3290:155135`, axis `Actions: Single|Double`) — applied identically on 3 existing frames + inherited by 4 more new Detail-tab frames via `clone()` of the already-fixed shell.

### icon-box-wrapper-shares-mastercomponent-across-different-glyphs
**Principle:** Two working `icon-box` instances with VISUALLY different glyphs (e.g. a green check-circle vs a red x-circle) may resolve `getMainComponentAsync()` to ONE AND THE SAME mainComponent id — a shared generic wrapper component. The glyph itself is switched by an INSTANCE_SWAP property deeper in the nested tree (`icon master component` → `Icon/<glyph-name>`), not by choosing a different mainComponent at the `icon-box` level. It follows that knowing the mainComponent id of one working instance you CANNOT reliably create an instance WITH ANOTHER glyph via `mainComponent.createInstance()` + guessing the nested INSTANCE_SWAP prop — it's more reliable to find an ALREADY WORKING instance with the needed glyph on the page and clone exactly that.
...
```
Verified on a production admin dashboard, one of the sections (an "Attach document" task — `Text button`/`2685:18749` vs donor `button`/`2629:21609`; the plan wrongly assumed "the same component, just setProperties").
...
```
Verified on a production admin dashboard, one of the sections (a bulk manifest — replacing the generic `Icon/check` with `Icon/switch-horizontal` / `Icon/Link-alternative` in 6 accordion headers).
...
```
Verified in a design-system token-cluster file (Row States Catalog, a preflight task) — the `Badge/Status` key from the implementation plan didn't resolve; two OTHER keys from the plan, genuinely external (`item-action` / `Icon/link-broken` from a production admin dashboard), imported with the same call without issues — the asymmetry pointed at "a local component", not "no access".
...
```
Verified in a design-system token-cluster file (Row Redesign v2, planning) — when planning the recolour step for a swapped kebab icon, it was decided to look for the vectors from `btnClone` as a whole, not via a repeat `findOne(name==='icon master component')`, precisely because of this uncertainty.
...
```
Verified in a design-system token-cluster file (Row Redesign v2, planning) — cloning an `icon-button` (a Sort-arrows chevron) from inside a live Header variant `.NavigationBar/AccountTrigger` for reordering relative to a new outer badge, without detaching the instance.
...
```
Verified in the same design-system token-cluster file (Cluster Popover v3) — an independent audit found that the `Token Header v2` master coloured its text with the wrong token; fixing the default could have affected a live instance of that master inside the owner's read-only playground node — comparing `boundVariables` showed the owner's instance already carried its own `text/primary` override, different from the master's current (wrong) `text/secondary` default — so the master fix was guaranteed not to change the playground, which the post-edit screenshot confirmed.
...
```
Verified on a production admin dashboard, one of the sections (the open state of one of the form modals, a comment text field): the DS has no separate Textarea, and neither does the code (`Input multiline` — the same React component). An `input-search` instance (label `HUG` + `Search Bar` `FIXED` 48h) was cloned; `Search Bar` resized `48→96` — the final screenshot showed a correctly enlarged field without clipping / a drifted label; the `content layout` wrapper (`HUG` height) recomputed automatically.
...
Verified on a production admin dashboard (row-level failure frames of a set of three action buttons) — the failure frame of the first action turned out to be a COMPONENT (cascade = a live INSTANCE `8840:4563`; the master fix propagated for free); the failure frames of the other two actions were ordinary FRAMEs (cascade = independent CLONEs `8869:5240`/`8869:5255`, named with a suffix `[CLONE — donor ..., updated]`), and needed the same structural rearrangement (toast beside the modal, not under it) by hand once more on each.
...
```
Verified in a design-system file, a Section States / RangePanel empty case — `detachInstance()` on a nested MetricBarRow (`Kind=best`) invalidated the RangePanel clone's ID captured one level above through the Histogram instance.
...
```
Verified in a product file (one of the search features — chip icons on the search settings screen + row icons on a new picker component) — reproduced twice independently (a provider logo inside a `Chip-right label` instance, a provider logo inside a `SelectList — account rows` row).
...
```
As a side effect it also solves another problem of the same case: semantically identical list rows (rows of one `SelectList`) may have unequal nesting depth among themselves (some rows — `Entity info → [logo, Name wrapper]` directly, some — with an intermediate `Icon wrapper`; `CheckBox box` also at varying depth) — a fixed `.children[N]` / a single-level `.children.find()` isn't universal for all rows of one list, while a non-descending BFS finds the needed node regardless of which level it landed on in a specific row.
Verified in a product file (one of the search features, picker-component rows, `SelectList — account rows` inside an `Account picker` clone) — the crash reproduced 3× in a row on one provider's row (8 nested VECTOR paths of its logo); after switching to the non-descending BFS all 5 rows (including 2 with an unforeseen `Icon wrapper`) were processed without a single error.
...
**Principle:** `minHeight`/`maxHeight` on an auto-layout frame lying INSIDE a live (not detached) INSTANCE (e.g. the inner field container inside a DS text-input component) can't be overridden — the same class of restriction as `resize()`/`appendChild` on nested instance children (see the neighbouring gotchas), only for `minHeight`/`maxHeight`.
...
**Principle:** The XML output of `get_metadata` for nodes nested inside an `<instance>` may show short "ordinary-looking" ids (`0:6`, `0:7`, `0:8`…) for the instance's inner override children — these are NOT real addressable ids. The real id format for instance-sublayer nodes is `I<instanceId>;<suffix>` (see the composite-id family of gotchas above in this file). Passing a `0:N`-style id directly into `figma.getNodeByIdAsync()` inside a `use_figma` script returns `null`.
**Symptom:** `get_metadata` returns a clean tree with normal-looking ids for TEXT/FRAME children inside an INSTANCE (e.g. the caption text of a reusable `dragdrop_block` component). A `use_figma` script built directly on those ids fails with `TypeError: cannot read property '<method>' of null` on the first line touching the node — the id silently resolved to nothing; the lookup itself throws no error.
...
```
Verified in a component library file — `get_metadata` on a `dragdrop_block` instance inside `TransferApprovalSheet` showed the caption text under id `0:8`; the next `use_figma` script with that id failed atomically (the whole script rolled back) before applying a single mutation — fixed by resolving the instance's own id and walking via `.findAll()`.

### empty-slot-socket-shares-name-with-its-own-default-filler-match-on-double-nesting-not-bare-name
**Principle:** A "socket" component for an instance-swap slot (the wrapper through which an icon/content is substituted into another component) may be named the same as its OWN unfilled default (in the verified case the socket and its default stub are both named `design-plug`). A search by the bare instance name (`node.name === 'design-plug'`) matches BOTH states — the empty slot and the correctly filled one (a filled slot is the same `design-plug` socket, just with a real icon INSIDE it rather than that same stub). On a canonical UI-kit file with a hundred+ live, correctly assembled icon slots such a search gives 100% false positives.
...
```
Verified in a component library file — a "Copy link" button (`4059:4103`) needed a copy icon; all 36 `preferredValues` of the `UI Icon#2011:0` slot didn't resolve. Nearby on the same page a "Copy" button was found with an already working `copy-06` icon (id `110:3202`, inserted via a separate, older direct-instance pattern without a socket wrapper) — `swapComponent` to it gave a pixel-identical size (20×20 before and after), a result visually confirmed by screenshot.
...
**Observation:** similar render instability (not only property reads) in the same file and area was recorded independently: `get_screenshot` on individual `Card-status` instances returned degenerate 1×1 px images with perfectly normal node geometry. May be the same class of file instability on repeatedly cloned / heavily overridden instances, not two different bugs.
Verified in a component library file — an attempt to hide a "User tag" chip (`Chip-right label` inside a `Tag group`, nesting `Card-status` → `Row-bottom` → `Chip group` → `Tag group`) on 6 cards of a `/disputes` list: the first direct dump confirmed the chip as live text; three subsequent attempts (a constructed `I<id>;501:5279` path, `findAllWithCriteria` by text, a manual `.children` walk) — three different failures (`get_visible`/`get_name` on phantom ids, an empty `Tag group` without a second child). The final live screenshot of the screen didn't show the chip at all — the finding wasn't forced; recorded as not reproduced.
...
Verified on a production admin dashboard — assembling a Team detail shell: between `text header` (title) and `actions group` a new node `MemberActiveToggle` (a status Tag + Switch) had to be inserted inside `Navigation bar/Admin` (`3290:155145`, `Show tabs=true`). A direct `appendChild` into `Main content` failed; `detachInstance()` of the outer instance solved it in one step; `breadcrumps` / `text header` / `tabs` / `actions group` remained working live instances (`swapComponent`/`setProperties` on them kept working unchanged).
...
```
Verified on a production admin dashboard — an attempt to cap a team title at 300 px (truncation) after the outer `Navigation bar/Admin` had already been detached earlier in the same session: seven consecutive attempts with different combinations (`textAutoResize`, resize/truncation order, `layoutSizingHorizontal` vs `primaryAxisSizingMode` on the intermediate `text layout`) consistently gave 173 px instead of 300 — it turned out `text header` (the title instance) was itself still a live INSTANCE, never detached separately. Detaching exactly it solved the task first time.
...
Verified on a production admin dashboard — an attempt to import a `Cursors / Pointer` cursor component (`Type=Pointer`) failed; direct cloning of an existing instance (`8580:3564`, page "08 List Page B") from another page of the same file worked without issues.
