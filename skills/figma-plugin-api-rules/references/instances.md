...
**Principle:** The XML output of `get_metadata` for nodes nested inside an `<instance>` may show short "ordinary-looking" ids (`0:6`, `0:7`, `0:8`…) for the instance's inner override children — these are NOT real addressable ids. The real id format for instance-sublayer nodes is `I<instanceId>;<suffix>` (see the composite-id family of pitfalls above in this file). Passing a `0:N`-style id directly into `figma.getNodeByIdAsync()` inside a `use_figma` script returns `null`.
