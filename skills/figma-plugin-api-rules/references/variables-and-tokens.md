...
**Symptom of no harvest:** an attempt to derive all the derived slots by intuition / from scratch gives an inconsistent system (different alpha % for tokens of the same role, random hover directions — now darker, now lighter, without a single rule) and requires many rounds of visual correction, as in the first Brand A session (several "Superseded" edits after showing the owner).
